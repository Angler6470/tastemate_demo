      ...
      {keys.map(flavor => {
        ...
        return (
          <button
            key={flavor}
            ref={btnRef}
            className={`flex items-center gap-1 sm:gap-2 px-1.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm rounded-full border border-gray-200 dark:border-gray-700 shadow-sm transition-colors ${flavorGradients[flavor] || 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100'}`}
          ...
          >
            <span className="text-xs sm:text-lg" aria-hidden="true">{flavorIcons[flavor] || '🍽️'}</span>
            {flavor}
          </button>
        );
      })}
